...
/*    public function order(Request $request, Post $post, Knowledge $knowledge)
    {
        ...
        if($request->input('update') == 'like'){
            $test = DB::table('posts')->join('likes', 'posts.id', '=', 'likes.post_id')->orderBy('likes.like', 'DESC')->get();
        }
        dd($test);
        $post = $test;
        
        //return view('posts/index')->with(['posts' => $post->getPaginateByLimitLike(), 'knowledges' => $knowledge->ramdom()]);
        // クライアントインスタンス生成
        $client = new \GuzzleHttp\Client();
